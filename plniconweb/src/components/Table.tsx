...
function Table({ data, header, tipe, role}: { data: any; header: any; tipe: string; role: string }) {
  return (
    <>
      <div className="flex overflow-visible">
        <table className="w-full">
          <thead>
            <tr>
              {role == "admin" && tipe != "pm" && (<th className="h-auto w-auto border-collapse bg-blue-primary px-2 py-1 text-center font-normal xl:px-4">
                <p>Action</p>
              </th>)}
          ...
          <tbody>
            {Object.values(data).map((obj: any, idx: number) => {
              return (
                <tr key={idx}>
                  { role == "admin" && tipe != "pm" && (<td className="h-auto w-auto border-collapse border-b-2 border-bnw-500 px-2 py-3 text-center xl:px-4 flex gap-1 justify-center items-center">
                    {tipe != "user" && 
